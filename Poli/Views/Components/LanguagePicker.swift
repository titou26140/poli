...
struct LanguagePicker: View {
    ...
    var label: String = String(localized: "picker.target_language")
    var showLockIcons: Bool = true
        ...
        Picker(selection: $selection) {
            ForEach(SupportedLanguage.allCases, id: \.self) { language in
                HStack(spacing: 6) {
                    ...
                    if showLockIcons && !isPaid && !SupportedLanguage.freeTierLanguages.contains(language) {
