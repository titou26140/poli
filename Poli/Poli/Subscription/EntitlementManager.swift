...
final class EntitlementManager: ObservableObject {
    ...
    var dailyLimit: Int { currentTier.dailyLimit }

    /// Remaining actions for the current day.
    @Published var remainingActions: Int = Constants.dailyFreeLimit
    ...
    private init() {
        ...
        if let cached = UserDefaults.standard.string(forKey: Constants.UserDefaultsKey.cachedSubscriptionTier),
           let tier = SubscriptionTier(rawValue: cached) {
            ...
            self.remainingActions = tier.dailyLimit
